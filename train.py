...
class Train(object):
    ...
    def train(self):
        ...
        if self.init.resume_iters:
            ...
            self.model.restore_model(self.init,'train')
        ...
        for i in range(start_iters, self.init.num_iters):
            ...
            if (i+1) % self.init.log_step == 0:
                ...
                if self.init.use_tensorboard:
                    for tag, value in loss.items():
                        self.model.logger.scalar_summary(tag, value, i+1)
        ...
        for i in range(start_iters, self.init.num_iters):
            for dataset in ['CelebA', 'RaFD']:
                ...
                if (i+1) % self.init.log_step == 0:
                    ...
                    if self.init.use_tensorboard:
                        for tag, value in loss.items():
                            self.init.logger.scalar_summary(tag, value, i+1)
